...
data_positive = pd.read_csv('data/data_positive.csv', encoding='latin1')
data_RFM_raw = pd.read_csv('data/data_RFM_raw.csv', encoding='latin1')
data_RFM_mean = pd.read_csv('data/data_RFM_mean.csv', encoding='latin1')
data_RFM_with_id = pd.read_csv('data/data_RFM_with_id.csv', encoding='latin1')
data_RFM_train = pd.read_csv('data/data_RFM_train.csv', encoding='latin1')
# Model 
...
if choice == 'Introduction':    
    ...
    st.subheader("images/Customer Segmentation") 
    # Hiển thị hình ảnh từ file
    image = open("images/Customer_Segmentation.png", "rb").read()
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    image = open("images/revenue_by_country.png", "rb").read()
    st.image(image, use_column_width=True)
    st.write("""### **Nhận xét:**
#### - **Các khách hàng lớn thường tập trung ở UK, đây là một thị trường lớn cần tập trung vào quốc gia này.**""")
    ...
    image = open("images/revenue_by_month.png", "rb").read()
    st.image(image, use_column_width=True)
    st.write("""### **Nhận xét:**
...
#### - **Từ đó, ta có thể thấy được các tháng cuối năm sẽ là các tháng cực kì hot, cần tập trung vào những tháng cuối năm để có thể tăng thêm doanh số.**""")
    ...
    image = open("images/Unsupervised_Segments.png", "rb").read()
    st.image(image, use_column_width=True)
    image = open("images/rfm_scatter_unsupervised.png", "rb").read()
    st.image(image, use_column_width=True)
    st.write("""### **Nhận xét:**
